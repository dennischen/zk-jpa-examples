...
public class Item2CatViewModel implements Serializable{
	...
	public void setSelectedItem(Item selectedItem) {
		if(selectedItem!=null){
			...
			//to keep the categories of the item for remove/add relationship when updating.
	...
	public void update(){
		/**
		 * At here, We have to maintain the bi-direction,
		 * because 
		 * 1. The relationship maintainer is Category,
		 * so if you just update Item, the relationship will not change in DB.
		 * 2. No matter you need object-bi-direction in this page or not, you have to 
		 * you have to sync the Category bi-direction relationship to update the relationship in DB.
		 **/ 
		
		//NOTE, for non-maintainer side, if you update it, will cause jpa reload the relation-ship form db.
		...
		for(Category cat:selectedItemCategories){
			if(!currCategories.contains(cat)){
				cat = commonDao.reload(cat);//the cat is probably detached, reload it.
				cat.getItems().remove(selectedItem);//remove bi-direction relation-ship;
		...
		for(Category cat:currCategories){
			if(!selectedItemCategories.contains(cat)){
				cat = commonDao.reload(cat);//the cat is probably detached, reload it.
				cat.getItems().add(selectedItem);//add bi-direction relation-ship;
